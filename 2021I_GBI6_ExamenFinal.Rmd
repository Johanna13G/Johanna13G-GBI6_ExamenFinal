...
setwd("C:/Users/JOHANNA/Desktop/U/6to semestre/Bioinformática/GBI6_ExamenFinal")
getwd()
...
tabla_genes <- df_long %>% group_by(gen) %>% mutate(row = row_number()) %>%
    tidyr::pivot_wider(names_from = gen, values_from = expresion_level) %>%
    select(-row) 
descr(tabla_genes) %>% tab_df(title = "Expresion de genes para los diferentes tipos de cancer",
...
Como se puede observar en la tabla obtenida, el menor valor de sd correponde al gen GADPH, por tanto indica que la mayor parte de los datos de la muestra tienden a estar agrupados cerca de su media. Por otro lado, el valor más alto de sd correponde a GATA3 lo que indica que los datos se extienden sobre un rango de valores más amplio.También se puede observar que los valores más altos de la media y la mediana corresponden a MUC1 sin embargo al ser valores diferentes puede indicar la existencias de valores atípicos.
```
...
```{r list_regplots}
# Escriba el código para generar la lista. 
genes <- reg_gen_expression("GATA3")
```
