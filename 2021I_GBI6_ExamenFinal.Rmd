...
cancer_mama <- df_long %>% group_by(gen) %>% mutate(row = row_number()) %>%
    tidyr::pivot_wider(names_from = gen, values_from = expresion_level) %>%
    select(-row) 
descr(cancer_mama) %>% tab_df(title = "Expresion de genes para los diferentes tipos de cancer",
...
tcga_boxplots <- df_long[, -2]
library(tidyverse)
ggplot(tcga_boxplots, aes(x = dataset, y = expresion_level)) + geom_boxplot() + 
